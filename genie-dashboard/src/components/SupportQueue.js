        ...
        <tbody className="text-sm">
          {events.map((event, i) => (
            <tr key={i} className="border-b border-slate-700/20 hover:bg-white/5 transition-all">
              ...
              <td className="py-4 text-info font-mono text-xs">{event.leading_indicator}</td>
              <td className="py-4 text-slate-500 font-mono text-xs">{event.mttr}</td>
              <td className="py-4 text-right">
                <button className="btn-resolve text-[10px] uppercase font-bold border border-success text-success px-4 py-1 rounded hover:bg-success hover:text-white transition-all">
                  Resolve
                </button>
              </td>
            </tr>
